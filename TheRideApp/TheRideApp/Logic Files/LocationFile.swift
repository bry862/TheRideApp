...
class LocationClass1:NSObject, ObservableObject, CLLocationManagerDelegate{
    ...
    var passenger_location_placemark: CLPlacemark?
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        if let passenger_location_object = locations.last {
            ...
            callReverseGeocode(TargetLocation: passenger_location_object) { (x: [CLPlacemark]?, y: (any Error)?) in
               self.passenger_location_placemark = x?.first
            }
        }
    ...
    func callReverseGeocode (TargetLocation: CLLocation, _ completionHandeler: @escaping ([CLPlacemark]?, (any Error)?) -> Void) {
        
        GeocodeManager.reverseGeocodeLocation (TargetLocation,completionHandler: completionHandeler)
    ...
    func getPassengerLocationPlacemark () -> CLPlacemark {
        return passenger_location_placemark!
